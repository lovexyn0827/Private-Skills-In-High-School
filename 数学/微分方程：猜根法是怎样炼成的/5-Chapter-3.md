...
> 当初求解这一方程是为了验证简谐运动的运动方程，套用了结果的形式，所以便直接得出这样一个方程。
...
$$
这样便不难看出$k=\omega$，于是方程(3.2)的一组解为
$$
...
这里求通项的一个关键是由构建的等比数列的递推式求出等比数列通项的过程$A_{n+1}=kA_n\Rightarrow A_n$，这一原理在微分方程当中也有所体现，即我们在前面已经得到了通过形如$F'(x)=kF(x)$的方程得到$F(x)$表达式的方法。接下来，我们来仿照求数列通项的过程求解方程(3.16)：
